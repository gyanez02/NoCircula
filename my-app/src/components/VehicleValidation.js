...
const VehicleValidation = ({plateNumber,selectedDate})=>{
    ...
    useEffect(() => {
        axios.get(baseURL+'/'+plateNumber+'/'+formatted_date)
            .then((response) => {
    ...
    if(values.plateNumber == ""){
        ...
        if(values.canDrive == true){
            label = <label>You can drive without restrictions that day</label>;
            
        }
        else{
            label = <label>You can't drive that day</label>;
